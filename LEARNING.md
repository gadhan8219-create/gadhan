...
### סיכום כמותי — עמודת "סה״כ מאופסן" (2026-06-17)
ל"סיכום כמותי" (`WeaponsInventoryPage`, תצוגת qty) נוספה עמודה **סה״כ מאופסן** = מספר ה-serials עם `is_zeroed=true` לאותו פריט (helper `zeroedFor(itemId)`, ממוין לפי שם החייל). לחיצה על המספר פותחת `zeroedModal` — טבלה: חייל · צ׳ (— לפריטי כמות) · הערת איפסון (`zeroed_note`). השאילתה ב-`load` כוללת כעת `is_zeroed, zeroed_note`; `SerialRow` הורחב בהתאם. `colSpan` של שורת הריק עודכן `units.length + 3` → `+ 4`.
