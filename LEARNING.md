...
| דף | נתיב | תיאור |
|----|------|--------|
| AttendanceReportPage | /personnel/attendance | דיווח דוח 1 בזרימת status-first: מסגרת → תאריך → בחירת סטטוס → סימון חיילים רלוונטיים → סיכום חי → אישור (פעיל רק כשכולם סומנו). תמיכה ב"סמן צוות". עריכת תאריך קיים. **צוותים ב-collapse — סגורים כברירת מחדל** (`openTeams`, אותו דפוס כמו כניסות/יציאות; מתאפס בכל שינוי מסגרת/תאריך). |
...
- **תקני שדות טפסים (אחידות מובייל + דסקטופ, `index.css`):** `.input` הוא `text-base md:text-sm` — 16px במובייל **מונע זום אוטומטי של iOS** בפוקוס; viewport קיבל `maximum-scale=1.0`. `select.input` עם `appearance-none` + chevron מותאם משמאל (RTL) — חל גם בדסקטופ. `input[type=date].input` עם `appearance-none` + יישור ערך לימין; ה-`min-height` של ה-date הוא **mobile-only** (`@media max-width:767px`) כי iOS מקריס את גובה ה-date — בדסקטופ הוא ממילא בגובה שאר השדות, וה-min-height היה הופך אותו לגבוה מהם. כל ה-select/date משתמשים ב-`.input` ולכן אחידים.
...
> מיגרציה 0040 הורצה **ידנית** דרך Dashboard (הרשת חסמה את ה-CLI). היסטוריית ה-CLI עדיין לא מסונכרנת — בפעם הבאה על רשת פתוחה: `supabase migration repair --status applied 0040` (או פשוט `db push`, המיגרציה idempotent).
