...
interface SelectedItem {
  serial: string;   // for has_serials = true
  quantity: string; // for has_serials = false
}
...
export default function WeaponsCheckoutPage() {
  ...
  const [serials, setSerials] = useState<Record<string, string[]>>({}); // itemId → serial[]
  ...
  useEffect(() => {
    async function load() {
      const [it, ser] = await Promise.all([
        ...
        supabase.from('weapons_item_serials').select('item_id, serial_number').order('serial_number'),
      ]);
  ...
  function toggle(item: WeaponsItem) {
    setSelected((prev) => {
      ...
      if (next[item.id]) {
        ...
        next[item.id] = { serial: '', quantity: '1' };
      }
      return next;
    });
  }
                ...
                return (
                  ...
                  >
                    ...
                    <label className="flex items-center gap-3 p-3 cursor-pointer">
                      ...
                      {item.description && (
                        <span className="text-xs text-slate-400 hidden sm:block">{item.description}</span>
                      )}
                    </label>
                        ...
                        {item.has_serials ? (
                          <>
                            ...
                            <input
                              ...
                              dir="ltr"
                              placeholder="הזן או בחר מהרשימה"
                            ...
                            {itemSerials.length > 0 && (
                              <datalist id={`serials-${item.id}`}>
                                {itemSerials.map((s) => <option key={s} value={s} />)}
                              </datalist>
                            )}
                            {itemSerials.length === 0 && (
                              <p className="text-xs text-slate-400 mt-1">אין צ׳ים רשומים לפריט זה</p>
                            )}
                          </>
                        ) : (
                          <>
